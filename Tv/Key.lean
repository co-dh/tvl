...
def chR : UInt32 := 114      -- 'r' list directory (recursive)
def chRR : UInt32 := 82      -- 'R' list directory (non-recursive)
...
def ret (c : KeyCtx) (s : State) : KeyResult :=
  match c.v.vkind with
  | .freqV cols => Freq.ret c.v.query.render c.v.nav.rowCur cols c.v s
